...
| tick 频率 | 按原版：CK 行为帧 1/66 s。会话锚点把物理 time factor 重置为 1.0（`m_PhysicsTimeFactor = 0.001`），之后**关卡脚本照常驱动它**，两端因此完全一致：锚点后所有关卡都由 `Gameplay_Ingame` 的 init 写 2.0（每 tick 2 个 PSI，IVP `delta_PSI_time` 仍为 1/66 s；实测 Level 1/2/8 均如此），只有 Level 1 的 `Gameplay_Tutorial` 在教程期间写 0（冻结约 24 s，两端一起冻结）；tick 边界与 PSI 边界始终重合。会话期间**世界从不暂停**：ESC 只打开菜单 UI（输入按全零处理），暂停链对 `Gameplay_Ingame`/`Gameplay_Events` 的 `Deactivate Script`/`Activate Script` 目标在会话期间被清空（见 9.2 的暂停语义），物理与机关脚本照常步进；时钟守卫（API v8 `set_clock_guard`）的暂停传感器（`Gameplay_Ingame` 的非激活态）因此永远不会触发，它每帧只采样因子、一次都不写（详见 9.2）。 |
...
| 关卡开始：`Gameplay_Ingame` 激活 → `Init Ingame`（Set Physics Globals：重力 0,0,-20、时间因子 2——会话期间该写入照常生效，两端一致；暂停菜单的 0 由暂停链参数改写变成空操作（见 9.2）。**会话期间这个脚本在 ESC 菜单打开时也保持激活**：暂停链对它的 `Deactivate Script` 目标被清空（同见 9.2），因为 `BallManager` 的 `DepthTestCubes` 死亡检测就在这个脚本里，菜单期间球离开关卡同样要死；Execute Script `Gameplay_SectorManager` 激活分节 1）→ `BallManager/New Ball`：Set World Matrix(CurrentLevel[0,3] 复活点) → Delayer 3 s → `physicalize new Ball` → Physics WakeUp。 | `Gameplay_Ingame` 图 |
...
- **时间线**：客户端锚点 = 会话开始后重开关卡并首次看到 `Gameplay_Ingame` 激活的那个 tick，它的编号由服务端分配（`SessionAssign`）：会话开始时在场的成员拿到**会话起始基数**（`input_delay + min(ceil(最差成员 RTT_ms × 66 / 1000), 12) + 2`，再夹到 `44 − input_delay`、下限 1，见 9.2 的"编号要领先"），迟到加入者与重同步者拿到服务端当前 tick 加同一个前导量。客户端收到分配即用 `rebase_tick` 换号：锚点之后、分配之前那段按自己锚点编的帧号连同修正历史一起丢弃，此后每个行为帧一个 tick（固定 1/66 s，`fixed_tick_driver` 节拍）。服务端在所有成员 `SessionReady` 后开始推进；tick T 在收齐所有成员 T 的输入、或服务端墙钟到达 `tick0_wall + (T + input_delay)/66 s` 时模拟，缺失输入沿用该玩家上一 tick 的输入。各客户端锚点的墙钟时刻可能相差 1–3 s（重开耗时）；编号基数统一之后这只剩下各自的半程网络延迟，不再随加载差放大快照与历史的差值。
...
- `server.cpp` 集成：`Start(mode=Physics)` 校验（`physics.enabled`、`game_root`、全员同一原版关卡、Mod 白名单——`ModList` 登录时已收到，存入 `client_data.mods`）→ 房间进入 `Starting` → 模拟线程 boot 世界 → 就绪后发 `SessionStart` → 收 `SessionReady`：比对锚点哈希与表面签名，不一致则 `SessionEnd(reason)`；全员就绪 → 开始推进 → `SessionInput/SessionEvent` 转入队列 → 快照回调直接用 `interface_->SendMessageToConnection`（GNS 线程安全）发给房间成员 → 离开/断线 → `remove_player` 并广播 `PlayerLeft`；房主 `Close`/`End` 或全员离开 → 销毁世界 → `SessionEnd`。迟到加入：房间 Running 时 `Join` 允许，服务端发 `SessionStart`，客户端重开关卡并锚点后发 `SessionReady`，服务端以 `SessionAssign{first_tick = 当前 tick + 前导量}` 回复（在场成员用会话起始基数，见 9.2 的时间线；重开耗时 1–3 s，因此不能在 SessionStart 里预先给号），客户端此后按 `first_tick + 锚点后帧数` 编号，旧的锚点帧号不再补发；服务端再补发所有在场玩家最后一次 Physicalize 事件与 full 快照；不做哈希校验。重开：房主再次 `Start` 相当于 `End` + `Start`。
...
8. 会话中 ESC 暂停：暂停菜单不再停摆世界——菜单只出现 UI，世界继续按 66 Hz 步进（本地物理与远端球都照常推进），暂停期间本地输入按全零处理，退出菜单即恢复，也不再触发 resync（见 9.2）。M3 期间曾是"本地物理停摆、恢复后由修正拉回"的已知限制，已由 9.2 的暂停链参数改写与脚本状态清空（`pause_scripts_*` 清空 `Deactivate Script` 目标，世界脚本保持激活）解决。
...
**开始对齐**：`SessionAssign` 对会话开始时在场的成员改为**全员准备好后一起发**（迟到者仍即时发）；无头会话客户端在收到分配前不推进帧。原因：锚点早的客户端会在等待别人期间领先服务端几十 tick，转发输入对它就滞后同样多（联调里无头客户端领先 38 tick，远端预测几乎失效）。原版客户端不能停帧，于是**由服务端给一个共同的编号基数**：全员一起收到 `first_tick = 会话起始基数`（时间线一节的公式，等于迟到加入者拿到的前导量，因为世界此时还没 tick），客户端收到即 `rebase_tick` 换号，锚点之后按自己锚点编的那段帧号与修正历史一起丢弃。这样两台同时重开关卡、锚点相差不到一秒的原版客户端，以及关卡加载慢上数秒的成员，编号都落在同一个基数上：本地历史与快照 tick 的差值不再等于“自己等人等了多久”，而是基数加上 input_delay 与单程延迟，加载差只影响各自锚点到分配之间的那几帧（被丢弃）。基数本身有上限（34 tick），再按回滚窗口夹一次（`基数 ≤ 44 − input_delay`，见下一条），所以再慢的加载也换不成更大的回滚深度，起始滞后（基数 + input_delay）始终留在窗口内。
...
- **暂停**（ESC、失焦）：**会话期间世界从不暂停，只有菜单 UI 出现**。ESC（或失焦）打开原版暂停菜单：游戏进入 `IsPaused()` 状态，客户端把本地输入按全零处理（`frame.keys=0`、`INPUT_FLAG_PAUSED` 置位、导航副本停止轮询），退出菜单立即恢复——但**关卡脚本一律不停**：`Gameplay_Ingame`（世界每帧的工作，含 `BallManager` 的 `DepthTestCubes` 死亡检测）与 `Gameplay_Events`（关卡事件）在菜单打开期间保持激活，物理照常按 66 Hz 步进，机关脚本照常运行，两端状态继续一致。实现分两层：
  - **时间因子**（9.18）：暂停链写的 0 在源头被中和。会话锚点时把 `Event_handler` 的 `Pause Level`/`Unpause Level` 链里 `Set Physics Globals` 的「Physic Time Factor」输入改写成**当前物理时间因子**（每帧按 `get_clock` 刷新；该输入经 `CKIpionManager::SetTimeFactor` 的 `×0.001` 缩放，脚本值是正常时的 2.0，故写入的是引擎值 ×1000），暂停/恢复链执行时写进去的就是已经在用的值，对时钟是空操作；会话结束（`physics_session_end_local` / `OnPostExitLevel`）写回零售值。
  - **脚本状态**（`pause_scripts_*`，`session/physics_session_client.cpp`）：暂停链的 `Deactivate Script`（`Gameplay_Ingame`、`Gameplay_Events`）与恢复链对应的 `Activate Script` 的 **target 被清空**——写 0 到该 `Script` 输入的真实来源参数。两个块读到空目标就什么也不做（`Deactivate Script` 调 `CKScene::DeActivate(nullptr)` 直接返回，`Activate Script` 显式跳过空目标），而链路本身（块、连线、延迟）原样保留，所以菜单自己的效果（End Music、Mouse On/Off、Start Music、光标链、恢复链对 `Gameplay_Refresh` 的 reset+activate）全部照旧。解析在**每次关卡（重）进入**（锚点）做一次，每帧重新应用，会话结束写回零售目标；目标参数被本修复之外的输入共享时整份修改放弃并记 Error（不改图），保证不会悄悄半途生效。`Gameplay_Tutorial` 不在保护名单里：菜单停掉教程是零售行为、对世界无影响，恢复链也不带 reset。
  - **时钟守卫**（API v8 `set_clock_guard`）：它的暂停传感器正是 `Gameplay_Ingame` 的非激活态，而会话期间这个脚本不再被停掉，所以传感器**永远不会报告暂停**，守卫只剩采样分支（把当前因子抄进 `target` 再比较，恒等，不写任何值），既不钉因子也不干扰关卡脚本——**已失效但无害**。仍然保留：守卫在 `BallanceMMOCommon`（服务端共用），且 Mod 自己的帧钩子跑在物理步进之后，真出现新的暂停来源时只有它能当场看到。
  - **实测（2026-09-10，原版客户端 ESC 暂停 + 菜单期间球离开关卡，Level 11，双人会话，`build/pause-menu-20260910/REPORT.md`）**：菜单打开期间 `paused=1 ingame_script=1 events_script=1 muted=1 time_factor=0.002000`，`ivp_time` 每样本 +1.061（= 2.0 PSI/s）、`tick`/`fixedtick` 每样本 +35（66/s），`resyncs 0/0`、`mism`/`frozen` 不增、`max_err` 与暂停前一致；退出菜单后速率不变。**菜单打开时球离开关卡现在会死**：把球从起点平台开下去，球还在下坠时按 ESC，日志里 `On Message PauseLevel`（`input muted (ingame script 1, paused 1)`）之后 1.0 s 出现 `On Message BallOff`，再 1.0 s `own ball unphysicalized`，随后在原版复活点重新 Physicalize——全部发生在 `paused=1` 期间。对照（修复前同版客户端、同样的开球手法、菜单同样打开）：球越过死亡盒后一路掉到 y=-400 仍不死（`build/manual-test-922-20260910` 记录的原 bug——暂停窗口 ticks 7917-14925 内球滚出关卡，越过此前死过的深度却没被杀死）；修复前的另一组对照里，菜单开着时死亡一直压着，菜单一关 0.12 s 内就出现 `Ball Off`。会话结束（`mmo room close`）后表被清空、两条链重新指向 `Gameplay_Ingame`/`Gameplay_Events`，此时再按 ESC 又恢复原版行为（脚本被停、因子 0、世界冻结）。
- **节拍原点重设 / 长时间掉帧**：固定节拍驱动器落后超过 33 tick 时会重设节拍原点，此时客户端的 tick 编号与服务端脱节，仍走重同步：客户端向服务端发 `SessionResync{session, last_full_tick}`，服务端回 `SessionAssign{first_tick = 当前 tick + 前导量}` 并强制一次全量快照；客户端收到后把 `tick_base` 改为新值、`frames_since_anchor` 归零重排（后续帧号从新基数计）、清空所有修正历史（含机关权威行与字典），并在下一次全量快照时对所有刚体（自己的球、远端球、机关）硬置一次。输入历史里旧编号的帧丢弃。
...
- **编号要领先**：迟到加入、重同步与会话开始时分配的不是服务端"当前 tick"，而是 `当前 tick + 前导量`（前导量 = `input_delay + min(ceil(RTT_ms × 66 / 1000), 12) + 2`，上限 34；会话开始时"当前 tick"项为 0，即只有前导量本身，再按回滚窗口夹一次）。客户端按这个编号推进，才像会话开始时的成员一样领先服务端；按当前 tick 分配时客户端反而落后半个 RTT，每个快照到达时本地还没有那个 tick 的历史，全部 `unmatched`（首次联调正是如此，1380/1380）。代价是本地历史与快照 tick 的差值（也就是回滚要回放的 tick 数）从 `加载差 + input_delay + 单程` 变成 `基数 + input_delay + 单程`：加载差不再计入（这正是修复点），但最佳情况下也抬高了一个基数，所以**会话起始基数还要按回滚窗口夹一次**：`基数 ≤ 44 − input_delay`（44 = `rollback.hpp` 的 `max_resim_ticks` 48 减 4 tick 余量，下限 1，`late_tick.hpp` 的 `session_start_tick_base`），起始滞后因此不会是 `2 × input_delay + 往返项 + 2` 而是最多 44 tick，夹到上限时买的是窗口而不是领先——400 ms 链路（输入延迟 21、裸 RTT 27 tick）基数夹到 23，输入抵达余量 = 基数 + input_delay − 裸 RTT 仍有 17 tick；迟到加入与重同步拿的是未夹的前导量（上限 34 tick），边界见 `sim/late_tick.hpp`。
...
- IVP 的 PSI 时刻由环境里的双精度时钟推进，会话期间每 tick 的 PSI 数由关卡脚本写下的时间因子决定（锚点后所有关卡 2.0 → 2 个 PSI，Level 1 教程期间 0 → 不步进；两端一致，暂停菜单期间因子不变——暂停链参数改写把它变成空操作，见第 2 节与 9.2），tick 边界与 PSI 边界重合，绝对时间平移 K 个 tick 就是平移 K 个 tick 对应的 PSI 数、不改变分组；回滚**不倒拨绝对时钟**，重模拟让本地 IVP 时间比服务端多走 K 个 tick（只影响 mindist 事件表里的 float 相对时刻，产生的罕见差异由下一个快照再次纠正）。
...
- **本地物理时钟停止时的路径**：原版脚本会把物理时间因子设为 0（Level 1 的 `Gameplay_Tutorial` 在关卡开始后停约 24 s；暂停菜单），这段时间本地既不能预测也不能重模拟。引擎通过 `world.simulating()`（`get_clock` 因子 > 0）识别，不符时只写入权威状态、不重模拟、计入 `frozen`。同步开始的会话两端一起进教程、快照一致；迟到加入者在自己的教程期间靠这条路径贴住服务端。会话期间时钟守卫**不会解除教程冻结**：它每帧只采样因子、从不写入（会话期间 `Gameplay_Ingame` 不再被暂停链停掉，守卫的暂停传感器不会触发，见 9.2 的暂停语义），教程期因子为 0 时守卫照常采样为 0，因此这条路径仍然用于教程冻结；暂停菜单的 0 由暂停链参数改写与脚本状态清空抵消，不再走这条路径。
...
- **远端球镜像**：镜像刚体按配方建好之后，若这个玩家已有一张比事件更新的权威快照行，就用该行（位姿 + 速度）建体：Physicalize 事件比那条快照晚大约一个输入延迟到达，而那条快照里服务端已经把同点出生的球踢开了。9.17 用户反馈的出生抽动正是按事件自带出生位姿建体的结果——Level 8/11 五名玩家在同一 tick 同点 Physicalize，4 个镜像与自己的球完全重合，下一次物理步本地 IVP 炸开（600–1900 m/s、位置偏差 117 m），而服务端同一 tick 正常；每次 Physicalize 都 `invalidate_history()` 又让回滚在整个中继延迟窗口内没有历史可比，直到 30 条不匹配触发 resync、全量硬写，表现为弹飞 + 瞬移。权威行里的速度已经包含服务端的出生冲量，所以走这条路时不再补本地冲量（补了会翻倍）；没有比事件更新的行时（首个快照尚未到达）才退回按事件位姿建体并施加本地出生冲量。此后快照按 8.5 第 5 条的历史比较/渐变机制照常纠正剩余误差。
...
### 9.25 机关回到"本地模拟 + 回滚纠正"（撤销 Option A，并修掉它掩盖的四个缺陷）

**症状（用户 2026-09-09/10 反馈，Level 11/13）.** 三件事，都在球与机关接触时发生：①球先**穿进**石球/木箱，
随后被弹开或被瞬移纠正；②机关与机关之间同样先穿透再弹开；③球碰到沙袋（`P_Modul_26_Sack`）时整串
绳子+沙袋**抽搐**，球跟着被甩飞。

**根因（findings/A1–A5 的调查结论）.** 症状①②是 9.17 的 Option A（客户端不再模拟共享机关，每帧把机关
"渲染"到服务端行的外推位姿）本身的产物，③则是客户端自己的死亡重置在 body guard 之上制造的约束破坏：

- **D1 稀疏权威**：非全量快照只带**清醒**刚体（`physics_world::collect_bodies`，`!full && !simulated` 跳过），
  所以一个休眠的道具球在服务端被撞醒之前，客户端手里只有 32–98 tick 前的那一行。实测 client 2 的
  `P_Ball_Stone_MF003` 在 tick 3620 之前**一行都没有**，第一行出现时它已经以 1.78 m/s 在动——而那 ~21 tick
  里客户端把它按"冻结"钉在原地（冻结刚体仍然是碰撞体），预测的球撞的是一堵不会动的墙，服务端的球却早
  已把道具推开；下一份快照再把球往前纠正，正好纠进本地道具里。这就是"先穿透、再被弹"。
- **D2 外推不是模拟**：applier 沿行速度线性外推最多 0.6 s，摆动的绳子/沙袋因此偏 1–3 m；
- **D3 传送式写入**：外推/快照写入用 `set_body_state` 直接 beam，把机关瞬移进球里时不重建接触对，
  实测可把球以 15–33 m/s 弹出。
- **D4 死亡重置 + body guard**：客户端原版死亡链会 `Execute Script Gameplay_SectorManager`（Reset? = TRUE）
  重置本分节。引擎改动 #6 的 body guard 保住了刚体，却保不住**约束**：`Set Physics Ball Joint` 的 Create
  会删掉旧约束、按**参照实体当前世界位置**重建（`PhysicsBallJoint.cpp:105-149`），而同一条链里的
  `TT Restore IC(Hierarchy)` 刚把 `P_Modul_26_Balljoint_oben/unten`（未物理化，guard 管不到）放回初始矩阵。
  于是新的球窝锚点在初始位置、刚体却停在服务端给的位置上，约束在创建的瞬间就被违反了多少米，IVP 下一步
  用冲量把它抹平——这就是沙袋抽搐。A2 用原版 SimTool 在 HEAD 上复现过：guard 开启 + 分节重置，沙袋被推离
  自己的平面 6.7 m。
- **同时暴露出的三个老问题**：`set_body_state` 不恢复 IVP 的蛙跳增量（`delta_world_f_core_psis` 被 beam 清零
  又不重写，每次写入丢一个 PSI 的位移 |v|/66 m）；普通 beam 跳过 mindist 复检；`ensure_in_simulation`
  在已经清醒的刚体上会重置冻结计数（引擎改动 #14）；以及**每个生命周期事件都清空整份回滚历史**——实测
  client 2 的 21873 tick 会话里有 58 个这样的窗口（中位 23 tick、最长 47），合计 1306 tick = **6.0%** 的
  时间里没有任何刚体能被纠正。

**同样重要的是，Option A 当初的理由不成立.** 9.16 的 journal 显示：每个客户端在**自己死亡之前**都能把
沙袋预测到 0.0000–0.0008 m，几千 tick 不漂；发散总是从那次死亡开始。也就是说，"本地预测机关会抖"其实
是 D4，不是预测本身的极限。（另外两条支撑：`SetPhysicsForce` 与球窝约束都是**常驻的 IVP 对象**，不是每帧
的脚本写入，所以不跑脚本的重模拟照样保留它们；客户端的 CK 时钟本来就被 `fixed_tick_driver` 钉在与服务端
相同的 tick 长度上，脚本定时器同步推进。）

**决策.**

1. **机关重新变成普通的被跟踪刚体.** 删掉整套 Option A（`kMechanism*` 常量、`mechanism_target*`、
   `mechanism_candidates`、`physics_session_apply_mechanism_authority`、`physics_session_pose_mechanisms`
   与 `rollback_world::pre_step` 的那个 lambda、`mechanism_pose_history` 及其计数器）。客户端像模拟任何
   别的刚体一样模拟机关，快照行走回滚引擎的比较/恢复/重模拟。`pre_step` 钩子本身保留（给 9.x 的"脚本边沿
   重放"留口子），但客户端不再设置它。
2. **机关身份注册表**（新文件 `BallanceMMOCommon/include/session/mechanism_tracking.hpp`，原版 mod 与无头
   客户端共用）：按服务端 owner 序号存字典名 + 最新一行位姿 + 解析出的本地实体。同名多实例（Level 8 两个
   相距 332 m 的 `P_Modul_30_Wippe`）取"最新行离本地同名刚体最近"的那个 owner，超过 `identity_guard_m`
   （20 m）直接不认；本地没有这个刚体的名字（别的分节）解析为空、它的行被忽略。解析只在字典变大、本地
   刚体集合变化（桥接事件日志出现 created/deleted）或每 66 tick 发生一次，不是每行一次。
   `rollback_tracked::mechanisms` 取的正是"本地有刚体"的那批名字——名字列进去却没有刚体，会让每次
   `capture()` 都改变被跟踪集合。
3. **恢复规则改写**（`rollback.hpp`，见同一提交的 R 部分）：任何一个刚体越界时，**每一个有服务端行的刚体
   都从它自己的行恢复**（此前只恢复越界的那个、其余用本地记录，等于把两个世界的状态混在一起——对绳子↔沙袋
   这种受约束对，4 cm 的差就是一次约束违反，求解器用冲量抹平，下一份快照又越界，自激循环）；没有行的被
   跟踪刚体"不发表意见"（不比较、不冻结）；休眠行只有在同时越界时才写入并冻结；生命周期不再清空历史，
   改用**世代号**（`rollback_tracked::generations`，由调用方在 Physicalize/Unphysicalize/trafo/重生时 +1）
   区分"同名但不是同一个刚体"。
4. **写入方式**（桥接 API v10 `set_body_state_ex`，见同一提交的 E 部分）：睡眠状态与接触复检由调用方分别
   决定——大多数恢复用 `keep`（不碰刚体的模拟状态，也就不会重置冻结计数），行说醒才 `wake`、行说睡且越界
   才 `freeze`；**写入真的挪动了刚体（> 1 mm 或速度差 > 1 cm/s）时才做 mindist 复检**，否则优化过的 beam
   会把刚体放进别人身体里而不生成接触。这条路径同时补回蛙跳增量。
5. **中和两条原版链（N1/N2）：客户端不再替服务端重置世界.**
   - **N1 死亡重置**：`Gameplay_Ingame / BallManager / Deactivate Ball` 里那个 `Script = Gameplay_SectorManager`
     的 `Execute Script` 块，其 `Script` 输入的真实来源参数在会话期间被写 0（与 9.2 暂停链同一手法、同一套
     共享读者安全检查与每帧重应用，会话结束写回）。空脚本时该块**立即激活 Out**
     （`ExecuteScript.cpp:66-70`），所以 `Set Cell.Found -> Execute Script.In -> Deactivate Ball.Ball OFF`
     之后的复活链一切照旧。理由：服务端从来不跑这个"个人死亡重置分节"（第 2 节的决策），客户端跑了就是
     D4。
   - **N2 检查点只激活、不停用**：`Gameplay_Events / activate Sektor` 里写 `IngameParameter[0][2]`
     （Deactivate Sector）的那个 `Set Cell`，其 `Value` 输入在会话期间改接一个会话自己的 int 常量 0
     （原来的来源——直接来源或共享来源——记下来，会话结束还原并销毁常量）。`Gameplay_SectorManager` 的
     `Deactivate Sector` 组第一个块就是 `Test(Not Equal, B = 0)`，读到 0 直接跳过整段停用。这与服务端世界
     的"分节并集、只激活不停用"（`physics_world::update_sectors`）一致，也正是自动化动词 `sector` 与无头
     客户端 `--start-sector` 手写的那三个值。
6. **无头客户端补齐**（`BallanceMMOServer/sim/session_client.cpp`）：和 mod 共用注册表与世代号；
   `entity_of` 走注册表（此前无条件返回字典名，Level 8 上会驱动错的那个 `Wippe`）；`input_at` 改成**精确
   tick**命中（此前取"不晚于该 tick 的最近一帧"，会把该 tick 并不存在的按键边沿重放进去）；补上每帧的
   `amend_record`（此前完全没有，纠正会被下一次回滚原样抹掉）；重编号（assign/resync）时清空历史。
   另加两个给测试台用的开关：`--start-sector N`（锚点处激活第 N 分节，让后面的机关存在）与
   `--beam X Y Z TICK`（在该会话 tick 把自己的球放到那个位置）——没有一个关卡的复活点旁边有可动机关，
   不给这两个开关，录制回放永远碰不到机关。

**与原版仍然不同的地方（有意为之，需要在发布说明里写清楚）.**

- **个人死亡不再重置本分节**：死了之后本分节已经吃掉的道具不会重新出现，机关也不回到初始位姿——这与
  "共享世界、个人死亡不影响别人"的既有决策一致（第 2 节），服务端本来就是这么做的。
- **检查点只激活新分节，不停用旧分节**：旧分节的机关继续存在并被模拟，和服务端一样。代价是同时开着的
  机关更多（内存/CPU 可忽略：一个刚体一 tick 一条记录，96 tick × 21 刚体 ≈ 226 kB）。
- 这两条都只在**会话期间**生效，会话一结束就把图改回零售状态。

**没有做的（留给后续提交）.**

- **相位对齐（gap 5）**：脚本驱动的机关在两端的世界相位差着"tick base 减去 SessionAssign 之前已经跑掉的
  帧数"（手动测试与无头冒烟跑里都实测为 8 tick）。正确的做法是服务端从起始基数编号它的第一个世界 tick、
  客户端在锚点把世界按住直到拿到编号；本次提交不动它，因为它需要改协议时序而不只是客户端。
  **（已在 9.26 完成，见文末。）**
- **给约束块加 guard 的引擎改动**（曾计划编为 #15；#15 现在是 9.27 的诊断观察点）：N1/N2 之后，被保留的刚体身上的 `Set Physics Ball Joint` Create/Shutdown
  在会话期间根本不会再跑，没必要为此改引擎；而且那样会破坏任何合法地开关约束的原版脚本。
- **多分节共享占位**（A2 6.3）与**服务端 `BodyRevived` 去重**（A5 2h：五名成员的同一条沙袋唤醒被上报五次）
  仍然待办。

**验证（2026-09-11，含 9.26/9.27 之后的复测）.**

- `BallanceMMOMessageTests` 162/162（9.25 新增回滚引擎/修正阈值用例，9.26 新增 `SessionStartBase`）。
- 服务端回放：9.24 手动测试的 journal 4000 tick 在 9.25/9.26 之后 `matched=4000`；9.27 把沙袋首摆改成"首次游玩"
  方向之后，这份旧 journal 从 tick 7（沙袋出生）起分叉，属预期（见 9.27）；9.26 之后新录的服务端 journal 回放
  `matched=1314/1314`。SimTool `--restore-at` A/B 残差约 1e-7 m。
- 无头 `probe.py`：Level 11 双客户端空闲 30 s 的不符从 9.24 前的 36 降到 9.25 的 20/666，再到 9.26 的 5/644 与 22/655
  （剩下的全是斜面上自己球的 5 cm 容差漂移，沙袋 0 次，`mech_max_err=0.0000`）；边沿输入 Level 11 四人 147–161/930、
  Level 13 四人 58–71/660；beam 到道具球上、沙袋撞球只产生 2–5 cm 的修正，穿透样本 0。
- 原版双客户端（本机，`retail_test.py`）：空闲 30 s `mism` 1/1；场景跑（beam 到道具球落点、beam 进沙袋摆动路径、
  按键驾驶、被沙袋撞死后重生）：穿透样本 0、沙袋 0 次回滚、道具球撞击 1 次 1.4 cm 修正、被撞期间 0 次修正；自己球
  的 7 次修正里 3 次是重生时落在另一名一直闲置在起点的玩家球上（服务端把新球弹起 9 m/s、本地没有）的一次性
  0.29 m 修正。`pausechain` 读数 `death_reset=applied/now=0 sector_keep=applied/now=0`。
- 无头端没有 N1/N2，它自己的死亡仍会重置沙袋关节（已知，见上）。


### 9.26 会话起始的相位对齐（世界的第一个 tick 就是起始基数）

**症状.** Level 11 的沙袋在两端都被精确模拟（本地预测与服务端逐 tick 位差 0.0000 m），却每隔约 100 tick
被回滚引擎纠正 2-15 次，空闲 30 s 就有约 20 次（手动双客户端跑：500 tick 内 75 次沙袋回滚）。

**根因.** 沙袋挂在一个常驻的 `Set Physics Force` 上，由一个 1.5 s 的 `Delayer` 周期性翻转方向；翻转的时刻由
**世界自锚点起走过的步数**决定，而两端在同一个 tick 号上走过的步数并不相等。设计 9.2 起，起始成员统一从
起始基数 B 编号（`sim/late_tick.hpp` `session_start_tick_base`，实测 B = 8~12），但

- 服务端的世界从 0 开始计数，等到它的计数器走到 B 时，它已经从锚点走了 B 步；
- 客户端在 `SessionAssign` 到达时把锚点以来的 k 帧重编号丢弃（`rebase_tick`），它的下一帧才叫 B，此时它从
  锚点只走了 k 步（无头客户端 k=0，原版客户端 k=1）。

于是同一个 tick 号上，服务端的世界比客户端多走了 B−k 步（实测 8~11 tick）：客户端的 Delayer 每次都比服务端
晚 8~11 tick 翻转，钟摆在每次翻转处相差约 0.5 m/s，回滚引擎就在每次翻转处纠正一遍，永远不收敛。球不受影响，
因为球是由带位姿的显式事件创建的。

**做法（三处，缺一不可）.**

1. **服务端**：世界的第一个被模拟的 tick 就是 B——`physics_world::set_tick_index()` 在 `create_session` 里
   把计数器设成 B，0..B−1 从不被模拟（不是跳过时间，只是给第一步换个号）。日志、调度器、输入缓冲、快照节奏、
   黑匣子的 header `first_tick` 与 PLAYER 记录都从 `tick_index()` 取号，所以 journal 依旧自洽：新文件的第一条
   TICK 就在 B，且写有一条 NOTE `start base B`。
2. **原版客户端**：锚点帧的 `OnProcess` 里**原地等待**（`physics_session_hold_until_assigned`），直到 `SessionAssign`
   到达才返回——和无头客户端同一套语义：1 ms 轮询网络，**一帧都不跑**。第一版用 time scale 1e-6 让帧照跑、只是
   时间几乎不走，实测不够：脚本计时器确实不走，可行为图里**按帧计数**的连线照样推进（延迟 n 帧的连线 n 帧后就触
   发），分节脚本因此比服务端多跑了 62 帧（run 2 实测：沙袋从第一个 tick 起就不同相）。为了让等待循环能收到编号，
   `SessionAssign` 不再经 BML 定时器转到游戏线程（定时器只在帧间跑，等待发生在帧内），而是进会话自己的队列
   （`assign_queue`），游戏线程在每帧开头和等待循环里取。等待期间不计 tick、不记录输入、不写黑匣子的 TICK 记录；
   放开时把节拍原点前移，免得被误判成落后而触发重同步。10 s 等不到编号（或连接断开）就放开并告警——宁可相位
   错，不可卡死。
3. **无头客户端**：同样的语义（`--start-sector` / `--beam` 不受影响）。

放开之后，客户端的第一步和服务端的第一步都叫 B，两边在每个 tick 号上离各自锚点的步数完全一致。

**代价与限制.** 原版客户端在关卡起跑前画面停在最后一帧一小会儿（等待期间不渲染）：从本地锚点到拿到编号，
即最慢成员的 SessionReady 加一个可靠消息的往返（本机双客户端实测 11~36 次轮询，约 11~36 ms；服务端的起跑栅栏
把最慢成员封顶在 8 s）。**迟入者**仍然是老样子：它的世界从它自己的锚点开始，与服务端的相位差是任意的，
这一条仍然是已知限制。

**验证.** 无头 Level 11 双客户端空闲 30 s：沙袋回滚从 12 次（`P_Modul_26_Sack` 5 + `Sack001` 7，另有 5 条
kind 7 unmatched）降到 **0**，机关最大误差 0.0000 m；服务端新 journal 用 SimTool 回放 1314/1314 bit-exact；
旧 journal（first TICK 在 0）4000 tick 仍然 matched=4000。
原版双客户端（本机）：`session` 状态 `held=0/11..36`（无 `+timeout`）、`base=12` 且第一帧 `tick=12`、`rebases 0`；
沙袋在 9.27 之后 0 次回滚。

### 9.27 沙袋首摆的方向：Sequencer 的计数跨关卡重开保留

**症状.** 9.26 之后无头端沙袋 0 回滚，原版客户端却仍有约一半的会话里两个沙袋每 6 tick 被纠正一次，每次恢复之后
约 0.57 m/s 的速度差马上回来，永不收敛；同一局里两台客户端可以一好一坏（run 8/10/21/22），而且客户端一开
`session trace`（改变了帧时序）就"好了"。

**定位（要点，2026-09-11 二十余次本机双客户端跑）.** 逐 tick 的 exact 转储：出生 tick（19）之前两端逐位一致，出生
之后的第一个 PSI 起沙袋与绳子的四元数微小分量就**符号相反**、x 向速度相反、y 向相同。于是把出生瞬间的核心状态
（位姿、质量、惯量、阻尼、时钟字段、凸包哈希）、两个球关节的创建顺序与立即/延后、x87 控制字与 MXCSR、sim unit
里核心与控制器的顺序、出生后每个 PSI 的状态都记下来比对：全部一致，唯独结果两种（固定的两个 pose 哈希，从无第三
种）。`BMMO_SIM_ALLOC_PERTURB` 也能翻转结果，一度指向指针序，引擎审计（`findings/Q-ivp-birth-audit.md`）也没找到。
最后给 `SetPhysicsForce` 加了创建观察点才看见：出生时建出来的力方向相反（+0.25 与 −0.25）。

**根因.** 沙袋的 `Swing` 组在 `Swing.On` 时先让 `Sequencer` 触发一次，用它的输出决定先建哪一个方向的
`SetPhysicsForce`（Out 1 → 一个方向，Out 2 → 反方向），之后每 1.5 s 翻转。`Sequencer` 把当前位置存在输出参数
`Current` 里；`CKBehavior::Reset()`（重实现与原版一致）只清激活标志和连线延迟、不碰参数，所以会话开始的关卡重开
之后 `Current` 停在上一次游玩离开时的值——客户端取决于玩家在会话开始前已在关卡里待了多久（每 1.5 s 翻一次奇偶，
所以看起来随机、又对帧时序敏感），服务端取决于它自己的启动过程。关卡文件里的值是 −1（"尚未触发"）：首次游玩
先出 Out 1；之前的服务端启动碰巧把它推到 0（先出 Out 2），此前"干净"的客户端其实就是 `Current` 恰好也是 0 的
那台。回滚纠正对此无能为力：它恢复的是状态，差的是**力**。

**做法（三端）.** `BallanceMMOCommon/include/game/script_state.hpp` 的 `sequencer_state`：每一侧在关卡文件加载时
记录 `P_*` 对象脚本里每个 `Sequencer` 的计数（输出参数 0；老版本块用本地参数 0）——服务端与无头端在加载期间每
tick 扫一遍、首次见到即记（那时它的脚本还没跑过），原版客户端在地图的 `OnLoadObject` 之后的头几帧记（钩子本身
里行为还枚举不到）；锚点时写回。语义是"会话像首次进入关卡那样起动机关"，三端一样。

**验证.** 两台原版客户端交错 0.75 s / 1.1 s 进入关卡各跑一次：两台都记到 −1、锚点写回 −1，出生力 +0.25、tick 119
翻转 −0.25，与服务端一致；tick 19 的 pose 哈希三端相同（`9eee2492e417b822`）；`mism` 1/1 与 1/5（剩下的是出生
冲量期的对方球修正）。无头 Level 11 双客户端空闲 30 s 沙袋 0 次。单元测试 162/162。

**代价.** 服务端沙袋的首摆方向从 Out 2 变成 Out 1（首次游玩语义），9.24 的旧 journal 因此从 tick 7 起不再能回放；
新 journal 正常回放。

**限制.** 迟入者：它的 `Sequencer` 同样被写回文件值，而服务端的已经翻过若干次，方向可能相反，回滚纠正补不回来
——和迟入者的相位一起待做（需要把机关脚本状态放进全量快照）。本关卡其它带状态的块不受影响（`Delayer` 在
`Swing.On` 时重新起算，`TT Scaleable Proximity` 在 Create 时重新武装）；别的关卡若有其它跨重开保留的计数块，
要按同样办法处理。

**留下的诊断.** 桥接事件日志新增四种条目：`birth`（出生时的核心状态、凸包哈希、FPU 字）、`constraint`（球关节
创建：两刚体、立即/延后）、`force`（`SetPhysicsForce` 创建：方向、位置）、`psi`（设 `BMMO_PSI_PROBE=<名字前缀>`
